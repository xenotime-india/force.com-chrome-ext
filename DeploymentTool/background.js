...
function createPanel(title,table,tab) {
    $(tab).append('<li><a href="#'+$(table).attr('Id')+'-tab" role="tab" data-toggle="tab">'+title+'</a></li>')
    var topPanel = $('<div class="tab-pane" id="'+$(table).attr('Id')+'-tab"/>');
...
function createTable(fields,tableId) {
    var table = $('<table id="'+tableId+'" cellpadding="0" border="0" class="table table-striped table-bordered" cellspacing="0" width="100%"></table>');
    ...
    for (i = 0; i < fields.length; i++) {
        if(fields[i] == 'Select') {
            ...
            $(table).find('thead>tr').append('<th>' + fields[i] + '</th>');
        }
    }
    return $(table);
}
...
function maketypeblock(table_data,index,name) {
    var XMLString = '';
    if($(table_data).DataTable().rows('.selected').data().length > 0) {
        XMLString += '\n    <types>';
        $($(table_data).DataTable().rows('.selected').data()).each(function() {
            var current = $(this);
            XMLString += '\n        <members>';
            if(typeof(index) == 'number') {
                if($(this)[index] != '' && $(this)[index] != 'null' && $(this)[index] != null) {
                    XMLString += $(this)[index];
                }
            }
            else {
                for(i = 0 ; i < index.length ; i++) {
                    if($(this)[index[i]] != '' && $(this)[index[i]] != 'null' && $(this)[index[i]] != null) {
                        if(i != index.length - 1) {
                            XMLString += $(this)[index[i]] + '.';
                        }
                        else {
                            XMLString += $(this)[index[i]];
                        }
...
function makeobjectToRetrive(table_data, index, type) {
    ...
    if($(table_data).DataTable().rows('.selected').data().length > 0) {
        ...
        $($(table_data).DataTable().rows('.selected').data()).each(function() {
            if(typeof(index) == 'number') {
                if($(this)[index] != '' && $(this)[index] != 'null' && $(this)[index] != null) {
                    returnData.members.push($(this)[index]);
                }
            }
            else {
                for(i = 0 ; i < index.length ; i++) {
...
function showLoading() {
    if(!$('#status').hasClass('show')) {
        ...
        $('#status').show(); // will first fade out the sfdcConsoleloading animation
